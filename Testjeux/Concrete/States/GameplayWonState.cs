...
namespace GameNameSpace
{
	public class GameplayWonState : GameplayBaseState
	{
		...
		public override void Enter()
		{
			...
			objects = (Container as IScene).GetObjects(item => item is RacketGameObject || item is BallGameObject);
			...
			origin = new Vector2(destination.X, -300.0f);
			(Container as GameplayScene).RegisterGameObject(factory.DecorateEntrance(new ButtonGameObject(origin, "Continuer", Color.Black, new SwitchSceneCommand(SceneType.GAMEPLAY, (Container as GameplayScene).Level + 1)), origin, destination));
