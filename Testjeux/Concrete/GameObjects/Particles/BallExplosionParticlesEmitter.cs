	...
	public class BallExplosionParticlesEmitter : BaseParticlesEmitter
	{
		...
		public override void Emit()
		{
			...
			for (int i = 0; i < Number; i++)
			{
				...
				Services.Instance.Get<IParticlesService>().Register(Texture, Services.Instance.Get<ITweeningService>().Get(TweeningName.QuintOut), position - Size * scale * 0.5f, new Vector2(particleSpeed * (float)Math.Cos(particleAngle), particleSpeed * (float)Math.Sin(particleAngle)), scale, ttl, 0.0f, 1.0f, 0.0f, Vector2.Zero);
			}
		}
	}
}